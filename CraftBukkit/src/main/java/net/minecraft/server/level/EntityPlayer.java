...
public class EntityPlayer extends EntityHuman {
    ...
    public void sendChatMessage(OutgoingPlayerChatMessage outgoingplayerchatmessage, boolean flag, ChatMessageType.a chatmessagetype_a) {
        if (this.acceptsChatMessages()) {
            outgoingplayerchatmessage.sendToPlayer(this, flag, chatmessagetype_a);
        }

    }

    public void sendChatHeader(SignedMessageHeader signedmessageheader, MessageSignature messagesignature, byte[] abyte) {
        if (this.acceptsChatMessages()) {
            this.connection.send(new ClientboundPlayerChatHeaderPacket(signedmessageheader, messagesignature, abyte));
        }
    ...
    public void sendServerStatus(ServerPing serverping) {
        this.connection.send(new ClientboundServerDataPacket(serverping.getDescription(), serverping.getFavicon(), serverping.previewsChat(), serverping.enforcesSecureChat()));
    }
